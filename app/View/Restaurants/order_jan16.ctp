          ...
          <div class="divider"></div>
                    <form action="<?php echo $this->webroot;?>restaurants/confirm_order/<?php echo $order['Reservation']['id'];?>" id="orderform" class="push-down-15" method="post" onsubmit='$("#overlay, #PleaseWait").show();'>
                      <div class="form-group">
                        <div class="col-xs-12">
                        <input type="text" <?php if($this->Session->read('restaurant') && $order['Reservation']['order_type']!='delivery'){?>disabled=""<?php }else{?>required=""<?php }?> id="fullname" name="ordered_by" class="form-control  form-control--contact" placeholder="Name">
                        </div>                        
                      </div>
                      <div class="form-group">
                      <div class="col-xs-12 col-sm-6">
                        <input type="email" <?php if($this->Session->read('restaurant') && $order['Reservation']['order_type']!='delivery'){?>disabled=""<?php }else{?>required=""<?php }?> id="ordered_email" name="email" class="form-control  form-control--contact" placeholder="Email">                        
                      </div>
                      <div class="col-xs-12 col-sm-6">
                        <input type="text" <?php if($this->Session->read('restaurant') && $order['Reservation']['order_type']!='delivery'){?>disabled=""<?php }else{?>required=""<?php }?> id="ordered_contact" name="contact" class="form-control  form-control--contact" placeholder="Phone Number">
                        </div>
                        <div class="clearfix"></div>                        
                      </div>
                      <div class="form-group">
                          <div class="col-xs-12 col-sm-6">
                            <input type="text" <?php if($this->Session->read('restaurant') && $order['Reservation']['order_type']!='delivery'){?>disabled=""<?php }else{?>required=""<?php }?> id="ordered_on_date" name="ordered_on_date" class="form-control  form-control--contact" placeholder="Date">
                            <input type="text" <?php if($this->Session->read('restaurant') && $order['Reservation']['order_type']!='delivery'){?>disabled=""<?php }else{?>required=""<?php }?> id="ordered_on_time" name="ordered_on_time" class="form-control  form-control--contact" placeholder="Time">
                          </div>  
                          <div class="col-xs-12 col-sm-1" style="padding-top: 30px;">
                            OR
                          </div>
                          <div class="col-xs-12 col-sm-5" style="padding-top: 30px;">
                            <input type="checkbox" value="1" name="order_now" class="order_now" <?php if($this->Session->read('restaurant') && $order['Reservation']['order_type']!='delivery'){?>disabled="" checked=""<?php }?> /> ORDER NOW
                          </div>
                          <div class="clearfix"></div>    
                      ...
                      <div class="form-group">
                        <div class="col-xs-12">
                        <textarea name="remarks" placeholder="Additional Notes" <?php if($this->Session->read('restaurant') && $order['Reservation']['order_type']!='delivery'){?>disabled=""<?php }?>></textarea>
                        </div> 
                        <div class="clearfix"></div>                       
                      </div>
                      ...
                      <div class="col-xs-12">
                      <input type="submit" value="Submit Order" class="btn btn-success noprint" <?php if($this->Session->read('restaurant') && $order['Reservation']['order_type']!='delivery'){?>disabled="" onclick="return false;"<?php }?>  />
                      </div>
                    </form>
